 ...
 */
import { NflState, Player, StatsByPlayer, TeamSchedule, TrendingPlayer } from '@/types';
...
export const api = {
  ...
  players: () => fetchJson<Player[]>('/api/nfl/players', 60 * MINUTE),
  schedule: (season: string) => fetchJson<TeamSchedule>(`/api/nfl/schedule/${season}`, MINUTE),
  stats: (season: string, week: number) => fetchJson<StatsByPlayer>(`/api/nfl/weekly/stats/${season}/${week}`, 2 * MINUTE),
  projections: (season: string, week: number) =>
    fetchJson<StatsByPlayer>(`/api/nfl/weekly/projections/${season}/${week}`, 5 * MINUTE),
