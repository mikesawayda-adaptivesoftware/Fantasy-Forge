...
describe('draft picks', () => {
  ...
  it('values earlier rounds, nearer years and worse teams higher', () => {
    ...
    expect(pickValue(p1, { firstDraftSeason: 2027, standingsRank, teams: 2 })).toBe(4.8); // best team picks last
    expect(pickValue(p2, { firstDraftSeason: 2027, standingsRank, teams: 2 })).toBe(7.2);
    const future = picks.find(p => p.season === '2028' && p.round === 1)!;
    expect(pickValue(future, { firstDraftSeason: 2027, teams: 2 })).toBe(5.1);
    const second = picks.find(p => p.season === '2027' && p.round === 2)!;
    expect(pickValue(second, { firstDraftSeason: 2027, teams: 2 })).toBe(3);
  });

  it('discounts the second draft in the offseason (pre-draft league)', () => {
    const offseason = buildDraftPicks({ rosters, tradedPicks: [], firstSeason: 2026, seasons: 2, rounds: 1 });
    const [thisDraft, nextDraft] = [offseason.find(p => p.season === '2026')!, offseason.find(p => p.season === '2027')!];
    expect(pickValue(thisDraft, { firstDraftSeason: 2026, teams: 2 })).toBe(6);
    expect(pickValue(nextDraft, { firstDraftSeason: 2026, teams: 2 })).toBe(5.1);
  });
});
